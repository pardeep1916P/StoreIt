  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        console.log('🔍 Shared files loaded:', sharedFilesData);
      } catch (error) {
      ...
      const processedSharedFiles = (sharedFilesData?.documents || []).map((sharedFile: any) => {
        console.log('🔍 Starting to process shared file:', {
          fileName: sharedFile.name,
          hasSharedBy: !!sharedFile.sharedBy,
          sharedBy: sharedFile.sharedBy,
          documentKeys: Object.keys(sharedFile)
        });
        ...
        let normalizedType = sharedFile.type;
            ...
            console.log('🔍 Processing shared file:', {
              ...
              allFields: Object.keys(sharedFile),
              fullFile: sharedFile
            });
            ...
            console.log('🔍 Owner assignment:', {
              fileName: sharedFile.name,
              sharedBy: sharedFile.sharedBy,
              originalOwner: sharedFile.owner,
      ...
      console.log('🔍 Final processed files:', {
        ...
        sampleFile: allFiles.documents[0],
        allFileTypes: allFiles.documents.map(f => ({ name: f.name, type: f.type, isShared: !!f.sharedBy })),
        sharedFilesWithOwners: allFiles.documents.filter(f => f.sharedBy).map(f => ({ 
          name: f.name, 
          owner: f.owner, 
          sharedBy: f.sharedBy,
          hasOwner: !!f.owner,
          ownerType: typeof f.owner
        }))
      });
