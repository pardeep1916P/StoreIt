...
const FileUploader = ({ ownerId, accountId, className, showButtonOnly = false, onUploadStart, onUploadComplete }: Props) => {
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      console.log('🔍 Upload mode check:', { showButtonOnly, isMobile: window.innerWidth <= 768 });
      
      if (showButtonOnly) {
        // For mobile button-only mode, always use global popup
        console.log('🔍 Using global popup for mobile');
        ...
        console.log('🔍 Using local state for desktop');
      ...
      const uploadPromises = acceptedFiles.map(async (file) => {
        ...
        Promise.all(uploadPromises).then(() => {
          ...
          if (showButtonOnly) {
            ...
            setTimeout(() => {
              console.log('🔍 Clearing upload popup');
              setFiles([]);
  ...
  const { getRootProps, getInputProps } = useDropzone({ 
    ...
    onDropAccepted: (files) => {
      console.log('🔍 onDropAccepted called with:', files.length, 'files');
    },
    onDropRejected: (rejections) => {
      console.log('🔍 onDropRejected called:', rejections);
    }
  });
