...
export const InlinePreview = ({ file, isOpen, onClose }: InlinePreviewProps) => {
  ...
  useEffect(() => {
    if (file.type === 'video' && file.$id) {
      console.log('🔍 Video component mounted, getting streaming URL...');
      console.log('🔍 File details:', { fileId: file.$id, fileName: file.name, fileType: file.type });
      
      const getStreamUrl = async () => {
        try {
          setVideoLoading(true);
          console.log('🔍 Calling getVideoStreamUrl API...');
          const result = await getVideoStreamUrl({ fileId: file.$id });
          console.log('🔍 Video streaming URL API response:', result);
          setVideoStreamUrl(result.streamUrl);
          console.log('🔍 Video streaming URL set to state:', result.streamUrl);
        } catch (error) {
          console.error('Failed to get video streaming URL:', error);
              ...
              <div className="relative">
                <video
                  ...
                  onLoadStart={() => {
                    ...
                    console.log('🔍 Video source URL:', videoStreamUrl || file.url);
                    console.log('🔍 Video extension:', file.extension);
                    setVideoLoading(true);
                    setVideoError(false);
                  }}
                  onCanPlay={() => {
                  ...
                  onLoadedData={(e) => {
                    ...
                    console.log('🔍 Current video src:', e.currentTarget.src);
