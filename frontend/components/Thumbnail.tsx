  ...
  useEffect(() => {
    if (isVideo && url && videoRef.current) {
      ...
      const generateThumbnail = () => {
        console.log('🔍 Generating video thumbnail for:', url);
        ...
        if (ctx) {
          try {
            ...
            console.log('🔍 Video thumbnail generated:', thumbnailUrl.substring(0, 50) + '...');
            setVideoThumbnail(thumbnailUrl);
          } catch (error) {
            console.log('🔍 Video thumbnail generation failed due to CORS:', error);
      ...
      video.addEventListener('error', () => {
        console.log('🔍 Video thumbnail generation failed, using fallback icon');
