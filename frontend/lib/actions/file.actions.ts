...
export const uploadFile = async ({
  ...
  try {
    ...
    console.log('🔍 Full upload URL:', `${process.env.NEXT_PUBLIC_API_GATEWAY_URL}/upload`);
...
export const getVideoStreamUrl = async ({ fileId }: { fileId: string }) => {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to get streaming URL: ${response.status} ${errorText}`);
    }

    const result = await response.json();
    return result;
  } catch (error) {
    console.error('🔍 Get video stream URL error:', error);
    throw error;
  }
};
...
export const debugSharedFiles = async () => {
  try {
    ...
    console.error('🔍 Debug shared files error:', error);
    throw error;
  }
};
