...
const getAuthToken = async () => {
  ...
  console.log('🔍 getAuthToken - Session found:', !!session);
  if (session) {
    console.log('🔍 getAuthToken - Token length:', session.value?.length);
    console.log('🔍 getAuthToken - Token preview:', session.value?.substring(0, 50) + '...');
  }
  ...
  try {
    ...
    if (response.status === 401 && retryCount === 0) {
      try {
        ...
        console.error('🔍 Token refresh failed, redirecting to signin:', refreshError);
...
export const verifyOTP = async ({
  ...
  try {
    ...
    const result = await response.json();
    console.log('🔍 verifyOTP - Backend response:', result);
    ...
    if (result.token || result.idToken) {
      ...
      console.log('🔍 verifyOTP - Setting token in cookie:', token.substring(0, 50) + '...');
      ...
      console.log('🔍 verifyOTP - No token found in result:', result);
    }
...
export const signIn = async ({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (error.error) {
        if (error.error.includes('Invalid email or password') || error.error.includes('Incorrect username or password')) {
          errorMessage = 'Invalid credentials';
          errorTitle = 'Error';
        } else if (error.error.includes('User not found') || error.error.includes('No account found')) {
          errorMessage = 'Account not exists';
          errorTitle = 'Error';
        } else if (error.error.includes('verify your email') || error.error.includes('not confirmed')) {
          errorMessage = 'Email not verified';
          errorTitle = 'Error';
        } else if (error.error.includes('Too many failed attempts')) {
          errorMessage = 'Too many attempts';
          errorTitle = 'Error';
        } else if (error.error.includes('Password reset required')) {
    ...
    try {
      ...
      if (result && result.email && result.email !== 'user@example.com') {
        ...
        if (!result.username || result.username === 'User') {
          console.log('🔍 getCurrentUser - Username missing or default, using email as fallback');
          result.username = result.email.split('@')[0]; // Use email prefix as username
        }
        return result;
      } else {
        console.log('🔍 getCurrentUser - Invalid user data received:', result);
        return null;
      }
    } catch (error) {
